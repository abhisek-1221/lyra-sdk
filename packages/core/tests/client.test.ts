...
import { yt, YTError } from "../src/index";
...
describe("yt() factory", () => {
  ...
  it("url.isVideo works correctly", () => {
    ...
    expect(client.url.isVideo("https://youtube.com/playlist?list=PLxxx")).toBe(
      false,
    );
  });
  ...
  it("url.isPlaylist works correctly", () => {
    const client = yt("test-key");
    expect(
      client.url.isPlaylist("https://youtube.com/playlist?list=PLxxx"),
    ).toBe(true);
...
describe("error hierarchy", () => {
  it("all custom errors extend YTError", async () => {
    const { NotFoundError, QuotaError, InvalidURLError, AuthError } =
